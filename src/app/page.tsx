...
export default function Page() {
  return (
    <main className="container relative mx-auto scroll-my-12 overflow-auto p-4 print:p-12 md:p-16">
      <section className="mx-auto w-full max-w-2xl space-y-16 print:space-y-6 mb-9">
        <div className="flex items-center justify-between text-balance">
          <div className="flex-1 space-y-1.5">
            <h1 className="text-3xl font-bold">{RESUME_DATA.name}</h1>
            <p className="max-w-sm text-pretty font-mono text-base text-muted-foreground">
              {RESUME_DATA.about}
            </p>
            <p className="max-w-md items-center text-pretty font-mono text-sm text-muted-foreground my-2.5">
              <a
                className="inline-flex gap-x-1.5 align-baseline leading-none hover:underline"
                href={RESUME_DATA.locationLink}
                target="_blank"
              >
                <GlobeIcon className="size-3 text-orange-500" />
                {RESUME_DATA.location}
              </a>
            </p>
            <div className="flex gap-x-1.5 pt-1 font-mono text-base text-muted-foreground print:hidden">
              {RESUME_DATA.contact.email ? (
                <Button
                  className="size-8 bg-gray-300 text-blue-500 hover:bg-white hover:text-black"
                  variant="outline"
                  size="icon"
                  asChild
                >
          ...
          {RESUME_DATA.work.map((work) => {
            return (
              <Card key={work.company}>
                <CardHeader>
                  <div className="flex items-start justify-between gap-x-2 text-lg gap-y-2">
        ...
        <Section>
          <h2 className="text-2xl font-bold">Formação / Cursos</h2>
          {RESUME_DATA.education.map((education) => {
            return (
              <Card key={education.school}>
                ...
                <CardContent className="mt-2 text-base">{education.degree}</CardContent>
              </Card>
            );
          })}
        </Section>
